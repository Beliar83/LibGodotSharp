...
public class Methods
{
	...
	void AddAttributeMethods(GeneratorExecutionContext context, INamedTypeSymbol c)
	{
		var ms = c.GetMembers().
			...
			Where(x => x.GetAttributes().Where(x => x.AttributeClass.ToString() == "GodotSharpGDExtension.MethodAttribute").Count() > 0)
			.ToArray();
	...
	public void Generate(GeneratorExecutionContext context, INamedTypeSymbol c)
	{
		...
		for (var i = 0; i < methods.Count; i++)
		{
			...
			code += $$"""
			          			...
			          			GDExtensionMain.extensionInterface.classdb_register_extension_class_method.Data.Pointer(GDExtensionMain.library, __godot_name._internal_pointer, info);
			          """;
			code += "\t\t}\n";
		}
		...
		for (var i = 0; i < methods.Count; i++)
		{
			...
			for (var j = 0; j < method.arguments.Length; j++)
			{
				...
				{
					args += $"({arg.Item1})GodotSharpGDExtension.Object.ConstructUnknown(*(void**)p_args[{j}])";
				}
				else
				{
		...
		code += $$"""
		          	...
		          	static void CallFunc(
		          		...
		          		void* r_return,
		          		GDExtensionInterface.GDExtensionCallError* r_error
		          	) {
		          		GDExtensionMain.extensionInterface.variant_new_nil.Data.Pointer(r_return);
		          		var instance = ({{c.Name}})p_instance;
		          		switch ((int)method_userdata) {
		          """;
		for (var i = 0; i < methods.Count; i++)
