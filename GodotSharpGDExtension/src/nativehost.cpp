...
godot::LocalVector<extension_data> bind()
{
    ...
    //
    const string_t godot_sharp_extension_path = root_path + STR("DemoLibrary.dll");

    WARN_PRINT(godot::Variant(std::filesystem::exists(std::filesystem::path(godot_sharp_extension_path))));

    const char_t *main_dotnet_type = STR("DemoLibrary.DemoLibraryExtensionEntry, DemoLibrary");
    const char_t *bind_method = STR("Bind");
    // <SnippetLoadAndGet>
    // Function pointer to managed delegate


    typedef extension_data* (CORECLR_DELEGATE_CALLTYPE *main_bind_function)(GDExtensionClassLibraryPtr, interface_functions*, void(GDExtensionObjectPtr p_instance, const GDExtensionConstTypePtr *p_args));
...
//    dlopen(NULL, RTLD_LAZY | RTLD_LOCAL);
    ...
    auto main_extension_data = bind(godot::internal::library, &funcs, &test1);
